    ...
    <header className="fixed top-0 left-0 right-0 z-40 transition-all duration-300">
      ...
      >
        <div className="container mx-auto px-4 sm:px-6 flex items-center justify-between gap-2 xl:gap-3 max-w-7xl">
          ...
          {/* 1. Brand Identity (Logo on mobile, Logo + Title on desktop) */}
          <a href="#home" className="flex items-center gap-2 xl:gap-2.5 group focus:outline-none shrink-0 order-1">
            {/* Circular Logo Frame */}
          ...
          <div className="hidden lg:flex items-center gap-1 xl:gap-1.5 shrink order-2">
            {navLinks.map((link) => {
          ...
          {/* 3. Action CTA Button (Centered on mobile via order-2, far right on desktop via lg:order-3) */}
          <button
            onClick={() => onOpenBooking()}
            className="btn btn-primary text-xs xl:text-xs 2xl:text-sm py-2 px-3 xl:py-2.5 xl:px-3.5 shadow-lg font-bold whitespace-nowrap shrink-0 flex items-center gap-1.5 order-2 lg:order-3"
          >
            <Calendar size={14} className="shrink-0" />
            <span>Prenota consulenza</span>
          </button>

          {/* 4. Mobile / Tablet Menu Toggle */}
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="lg:hidden text-white w-10 h-10 rounded-xl bg-[#112240] hover:bg-[#1e293b] border border-white/10 flex items-center justify-center transition-all focus:outline-none shrink-0 active:scale-95 order-3"
